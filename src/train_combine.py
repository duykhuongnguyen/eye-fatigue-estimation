...
class DataSetFactory:
    ...
    def __init__(self):
        ...
        files = pd["path"].tolist()
        label = df["label"].tolist()

        random.shuffle(files)
        for i, file in enumerate(files):
            img = cv2.imread(file)
            synthesis_images.append(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY))
            synthesis_labels.append(label[i])
        ...
        synthesis_images = synthesis_images[validation_length:]
        synthesis_labels = synthesis_labels[validation_length:]
        ...
        self.synthesis_training = DataSet(transform=train_transform, images=images, labels=labels)
        self.synthesis_validation = DataSet(transform=val_transform, images=validation_images, labels=validation_labels)

        images = []
        labels = []

        files = list(map(lambda x: {'file': x, 'label':0}, glob.glob('../dataset/clf/eyeState0/*.jpg')))
        files.extend(list(map(lambda x: {'file': x, 'label':1}, glob.glob('../dataset/clf/eyeState1/*.jpg'))))
        files.extend(list(map(lambda x: {'file': x, 'label':2}, glob.glob('../dataset/clf/eyeState2/*.jpg'))))
        files.extend(list(map(lambda x: {'file': x, 'label':3}, glob.glob('../dataset/clf/eyeState3/*.jpg'))))
        files.extend(list(map(lambda x: {'file': x, 'label':4}, glob.glob('../dataset/clf/eyeState4/*.jpg'))))
...
def main():
    ...
    synthesis_training_loader = DataLoader(factory.training, batch_size=batch_size, shuffle=True, num_workers=1)
    synthesis_validation_loader = DataLoader(factory.validation, batch_size=batch_size, shuffle=True, num_workers=1)
    
    network = model.Model(num_classes=1).to(device)
    # if not torch.cuda.is_available():
    summary(network, (1, shape[0], shape[1]))

    optimizer = torch.optim.Adam(network.parameters(), lr=lr)
    criterion = nn.CrossEntropyLoss()
    criterion_synthesis = nn.MSELoss()
    ...
    for epoch in range(epochs):
        ...
        for i, (x_train, y_train) in enumerate(training_loader):
            ...
            (x_train_synthesis, y_train_synthesis) = next(synthesis_training_loader)

            y_predicted = network(x_train)
            y_predicted_synsthesis = network(x_train_synthesis, fc1_return=True)
            loss = criterion(y_predicted, y_train) + lmbda * criterion_synthesis(y_predicted_synsthesis, y_train_synthesis)
            loss.backward()
            optimizer.step()
